...
function joinUrl(url) {
	...
	var sharePageAction = getAbsoluteUrl("share.action?shareUrl=" + url);
	return sharePageAction;
}
/**打开我的分享*/
...
function getAbsoluteUrl(url) {
	...
	if (url.match("^/")) { // url.startsWith("/")
		var rootUrl = window.location.href.substring(0, window.location.href.indexOf(window.location.pathname));
		return rootUrl + url;
	}
	...
	// 没有包含<base href="..."> 
	if (!baseElements || document.getElementsByTagName("base").length == 0) {
		var curUrl = window.location.href;
		if (curUrl.match("/$")) { // curUrl.endsWith("/")
